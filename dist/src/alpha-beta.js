...
import { Piece } from "./cescacs.piece";
import { csmv, Game } from "./cescacs";
import { undoStatusArrayEquals } from "./cescacs.moves";
/* running object */
...
class Minimax {
    ...
    static maxAnswerTime = 135 * 1000;
    static maxDepth = 4;
    static maxQDepth = 8;
    static limitedDepth = 2;
    ...
    meanms = () => { return this._n > 0 ? this._totalms / this._n : Minimax.maxAnswerTime; };
    _done = true; //main loop flag: must be explitly set
    _endLoop = true;
    _answerReady = false;
    _qanswerReady = false;
    _n = 0;
    ...
    static findBest(grand, board) {
        const actualGame = new Game(grand, board);
        ...
        function moveOrderValue(amove) {
            ...
            if (currentKing.checked) {
                ...
                if (csmv.isCaptureInfo(move))
                    tmpResult += Piece.pieceValue(cscnv.getPieceKeyName(move.captured)) << 2;
                return tmpResult;
            }
        }
