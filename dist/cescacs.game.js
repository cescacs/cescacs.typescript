...
function removeSymbol(hexElement) {
    ...
    if (lastChild.nodeName == "use")
        hexElement.removeChild(lastChild);
}
var selecting = false;
function clickHex(hexElement) {
    if (!isExecutingAction() && !game.gameEnd) {
        if (clickHex.svg === undefined) {
            clickHex.svg = new Set();
            clickHex.item = null;
        }
        const lastChild = (hexElement !== null && hexElement !== void 0 ? hexElement : clickHex.hexElement).lastChild;
        ...
        if (selecting) {
            if (hexElement.classList.contains('highlight')) {
                ...
                if (piece != null) {
                    ...
                    gameStatus.innerHTML = dest + ' ERROR!';
            }
...
function clearClickHex() {
    ...
    if (svg != null) {
        ...
        document.getElementById("resultString").innerHTML = (_a = game.resultString) !== null && _a !== void 0 ? _a : '\xa0';
    }
}
function displayMoveStatus() {
    ...
    if (game.gameEnd) {
        ...
        if (moveText == "")
            moveText = "Ready " + (game.turn == "w" ? "whites" : "blacks");
...
function isExecutingAction() {
    if (boardTLPDStatus || loadNewGame || endingGameResign || endingGameDraw || showingThreated || showingThreats)
        return true;
    else
        return document.getElementById("TLPD").style.display != 'none'
...
function activateConfirmPromotion() {
    ...
    confirmButton.disabled = ((_a = selector.value) !== null && _a !== void 0 ? _a : "") == "";
}
...
function restoreGame() {
    ...
    if (game.isGrand == isGrandStored) {
        ...
        if (isGrandStored)
            parser.searchParams.append('grand', "");
        else
    ...
    if (!game.checked) {
        ...
        if (castlingContainer.style.display == 'none') {
            ...
            if (playerCastlingStatus.length > 1) {
                ...
                castlingContainerContent.style.display = "none";
